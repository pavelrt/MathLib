...
fileprivate func supertriangle<P:Abstract2DPoint>(_ vertices: [DelaunayVertex<P>]) -> [DelaunayVertex<P>] {
    ...
    return [
        DelaunayVertex(x: xmid - 20 * dmax, y: ymid - dmax),
        DelaunayVertex(x: xmid, y: ymid + 20 * dmax),
        DelaunayVertex(x: xmid + 20 * dmax, y: ymid - dmax)
    ]
}
...
public func delaunayTriangulate<P:Abstract2DPoint>(vertices: [DelaunayVertex<P>]) -> [Triangle<P>] {
    ...
    var open = [Circumcircle<P>]()
    var completed = [Circumcircle<P>]()
    var edges = [DelaunayVertex<P>]()
    ...
    let ignored: Set<DelaunayVertex> = [_vertices[n], _vertices[n + 1], _vertices[n + 2]]
    
    let results = completed.compactMap { (circumCircle) -> Triangle<P>? in
        
        let current: Set<DelaunayVertex> = [circumCircle.vertex1, circumCircle.vertex2, circumCircle.vertex3]
