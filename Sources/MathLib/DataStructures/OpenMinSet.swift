...
/// The top of the heap is the minimum score element.
public struct OpenMinSet<T: Hashable, S: Comparable> {
  ...
  public init(startingValues: [T] = [], scores: [T:S] = [:]) {
    ...
    for i in stride(from: heap.count/2 - 1, to: -1, by: -1) {
      sink(i)
    }
  }
  ...
  public mutating func decreaseKey(_ element: T, to newScore: S) {
    ...
    swim(indexDictionary[element]!)
  }
  
  public mutating func increaseKey(_ element: T, to newScore: S) {
    assert(scores[element]! <= newScore)
    scores[element] = newScore
    sink(indexDictionary[element]!)
  }
  ...
  private func isGreater(_ e1: T, _ e2: T) -> Bool {
    scores[e1]! > scores[e2]!
  }
  ...
  private mutating func sink(_ index: Int) {
    ...
    while 2 * index + 1 < heap.count {
      var j = 2 * index + 1
      if j < (heap.count - 1) && isGreater(heap[j], heap[j + 1]) { j += 1 }
      if !isGreater(heap[index], heap[j]) { break }
      swap(index, j)
      index = j
    }
  }
  ...
  private mutating func swim(_ index: Int) {
    var index = index
    while index > 0 && isGreater(heap[(index - 1) / 2], heap[index]) {
