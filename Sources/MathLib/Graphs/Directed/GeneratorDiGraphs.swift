...
public func createDiPath(length: Int) -> (IntDiGraph, IntVertex, IntVertex) {
  ...
  for _ in 1...length {
    ...
    _ = graph.addEdge(
      from: previousVertex.id,
      to: nextVertex.id,
      indexGenerator: &edgeIdGenerator)
...
public func createDiGrid<V: AbstractVertex, E: AbstractDiEdge>(
  dimensions: [Int],
  vertexMaker: @escaping ([Int], Int) -> V,
  edgeMaker: @escaping ([Int], Int, [Int], Int) -> E,
  initialGraph: DiGraph<V,E> ) -> (DiGraph<V,E>, ([Int]) -> Int)
{
    ...
    for dimensionIdx in dimensions.indices {
      ...
      if coordinate > 0 {
        ...
        let edge = edgeMaker(
          coordinates,
          indexer(coordinates),
          neighboorhCoordinates,
          indexer(neighboorhCoordinates)
        )
        graph.add(edge: edge)
      }
...
public func createDiGrid2(dimensions: Int...) -> (IntDiGraph, ([Int]) -> Int) {
  ...
  let (grid, indexer) = createDiGrid(
    dimensions: dimensions,
    vertexMaker: vertexMaker,
    edgeMaker: edgeMaker,
    initialGraph: initGrid
  )
  return (grid, indexer)
}
  ...
  loop { coordinates in
    ...
    for dimensionIdx in dimensions.indices {
      ...
      if coordinate > 0 {
        ...
        _ = grid.addEdge(
          from: vertexId,
          to: indexer(neighboorhCoordinates),
          indexGenerator: &edgeIdGenerator
        )
      }
      if coordinate < dimensions[dimensionIdx] - 1 {
