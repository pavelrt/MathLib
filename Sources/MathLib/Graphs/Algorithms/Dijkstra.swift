...
public func findShortestPathsDijkstra<G: AbstractDiGraph>(
  in graph: G,
  sourceId: G.V.Index,
  pathTo: [G.V.Index],
  lengths: @escaping (G.E.Index) -> Double,
  distancesTo: Set<G.V.Index>? = nil) -> (
  distances: [G.V.Index: Double],
  paths: [G.V.Index:[(edge: G.E.Index, vertex: G.V.Index)]])
{
  ...
  var openSet = OpenMinSet<G.V.Index, Double>() // PriorityQueue<DistanceVertexId<G.V.Index>>()
  //var orderId = 0
  
  func computeDistances() {
    openSet.push(sourceId, score: 0.0)
...
public func shortestPathAStar<G: AbstractDiGraph>(
  in graph: G,
  sourceId: G.V.Index,
  pathTo: G.V.Index,
  lengths: @escaping (G.E.Index) -> Double,
