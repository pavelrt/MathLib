...
public func createPath<VG: IndexGenerator, EG: IndexGenerator>(
  length: Int,
  indexer: inout GraphIndicesGenerator<VG,EG>) -> (IntGraph, IntVertex, IntVertex)
  where VG.Index == Int, EG.Index == Int
{
  guard length >= 0 else {
  ...
  for _ in 1...length {
    ...
    let edgeBetweenPreviousAndNext = IntEdge(
      id: indexer.nextEdgeId(),
      vertices: TwoSet(previousVertex.id, nextVertex.id)
    )
...
public func createCircuit<VG: IndexGenerator, EG: IndexGenerator>(
  length: Int,
  indexer : inout GraphIndicesGenerator<VG,EG>
) -> IntGraph where VG.Index == Int, EG.Index == Int {
  guard length > 0 else {
    fatalError("The length of the path has to be positive!")
  }
